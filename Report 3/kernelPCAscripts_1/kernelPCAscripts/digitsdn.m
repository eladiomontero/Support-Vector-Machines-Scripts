...
for k=1:lpcs;
 ...
 for i=1:ndig
   ...
   set(gca,'xticklabel',[]);set(gca,'yticklabel',[]);           
   if i==1, ylabel([num2str(round(sum(sum((Xdt - Xtest2).^2)/N),1))]); end
   drawnow    
 end % for i
end % for k
...
for k=1:lpcs;
 nb_pcs=npcs(k); 
 Ud=U_lin(:,(1:nb_pcs)); lamd=lam_lin(1:nb_pcs);
 ...
 for i=1:ndig
    ...
    if i==1, ylabel([num2str(round(sum(sum((Xdt_lin - Xtest2).^2)/N),1))]), end
 end % for i
end % for k
